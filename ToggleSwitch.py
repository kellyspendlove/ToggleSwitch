...
def toggle_rhs(t,state,pi):
  #Right hand side of Toggle System
  x,y = state
  return np.array([pi.A1 / (1+y**pi.B) - x, pi.A2 / (1+x**pi.C) - y ])

def plot_solution(pi,init_cond,t_final):
  X = integrate.solve_ivp(lambda t, y:toggle_rhs(t,y,pi), [0,t_final], init_cond)
...
def streamplot(pi,x_max,y_max,n = 100):
  ...
  for i in range(xv.shape[0]):
    for j in range(xv.shape[1]):
      x_vel[i,j],y_vel[i,j] = toggle_rhs(None,np.array([xv[i,j],yv[i,j]]),pi)
